...
// SendRegistrationConfirmation sends a "pendaftaran diterima, menunggu verifikasi" email.
func (n *Notifier) SendRegistrationConfirmation(ctx context.Context, email string, registrationID string) error {
	subject := "Pendaftaran Kamu Berhasil! — 40% of Heart Rate Run"
	body := fmt.Sprintf(`<p>Halo!</p>
<p>Pendaftaran kamu telah kami terima. ID Registrasi kamu: <strong>%s</strong>.</p>
<p>Tim kami akan memverifikasi bukti pembayaran kamu. Kamu akan mendapatkan konfirmasi lebih lanjut via email.</p>
<p>Salam,<br>Tim Amigos Care Club</p>`, registrationID)
